...
import { popupSuccessVisible } from '../popupstore.ts';

import './fullscreen.css';

export function PopupSuccess() {
    const $popupSuccessVisible = useStore(popupSuccessVisible);
    const handleClose = async e => {
        e.preventDefault();
        popupSuccessVisible.set(false)
    }

    return (
        $popupSuccessVisible ? (
            <div id="popup-success" className="fullscreen flex justify-center items-center">
                <div className="relative p-4 w-full max-w-md h-full md:h-auto">
                    <div className="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <button onClick={handleClose} data-modal-toggle="successModal" type="button" className="py-2 px-3 text-sm font-medium text-center text-white rounded-lg bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:focus:ring-primary-900">
                            Continuar
                        </button>
                    </div>
                </div>
